...
oauths_schema = OAuth2PasswordBearer(tokenUrl="/chapter06/token") # 请求Token的URL地址 http://127.0.0.1:8000/chapter06/token

@app06.get("/oauths_password_bearer")
async def oauths_password_bearer(token: str = Depends(oauths_schema)):
    return {"token": token}
...
def fake_hash_password(password: str):  #加密
...
async def get_current_user(token: str = Depends(oauths_schema)):
    user = fake_decode_token(token)
    if not user:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
            detail="Invalid authentication credentials",
            headers={"WWW_Authenticate": "Bearer"}, # OAuth2的规范，如果认证失败，请求头中返回“WWW-Authenticate”
        )
    return user
