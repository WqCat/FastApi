...
def path_params01():
    return {"message": "This is a message3"}

@app03.get("/path/{parameters}") #函数的顺序就是路由的顺序
def path_params(parameters: str):
...
def path_params_validate(
        num: int = Path(..., title="Your Number", description="不可描述", ge=1, le=10),  # 1-10之间
):
    return num
...
@app03.get("/query/validation")  # 长度+正则表达式验证，比如长度8-16位，以a开头。其它校验方法看Query类的源码
